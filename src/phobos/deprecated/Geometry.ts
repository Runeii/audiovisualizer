...
const colorToArray = (input: Color[]) => input.flatMap(value => [value.r, value.g, value.b])
const vector2ToArray = (input: Vector2[]) => input.flatMap(value => [value.x, value.y])
const vector3ToArray = (input: Vector3[]) => input.flatMap(value => [value.x, value.y, value.z]);
...
class Geometry extends EventDispatcher {
  static createBufferGeometryFromObject(object) {
    ...
    if (object.isPoints || object.isLine) {
      buffergeometry.setAttribute('position', new Float32BufferAttribute(vector3ToArray(geometry.vertices), 3));
      buffergeometry.setAttribute('color', new BufferAttribute(new Float32Array(colorToArray(geometry.colors)), 3));
  ...
  toBufferGeometry() {
    ...
    if (geometry.normals.length > 0) {
      buffergeometry.setAttribute("normal", new BufferAttribute(new Float32Array(vector3ToArray(geometry.normals)), 3));
    }
    if (geometry.colors.length > 0) {
      buffergeometry.setAttribute("color", new BufferAttribute(new Float32Array(colorToArray(geometry.colors)), 3));
    }
    if (geometry.uvs.length > 0) {
      buffergeometry.setAttribute("uv", new Float32BufferAttribute(vector2ToArray(geometry.uvs), 2));
    }
    if (geometry.uvs2.length > 0) {
      buffergeometry.setAttribute("uv2", new Float32BufferAttribute(vector2ToArray(geometry.uvs2), 2));
    }
